...
if (!Vue.config.$nuxt) {
  ...
  Vue.config.errorHandler = async (err, vm, info, ...rest) => {
    ...
    if (vm && vm.$root) {
      const nuxtApp = Object.keys(Vue.config.$nuxt).find((nuxtInstance) => vm.$root[nuxtInstance]);
...
async function loadAsyncComponents(to, from, next) {
  ...
  try {
    if (this._queryChanged) {
      const Components = await resolveRouteComponents(to, (Component, instance) => ({ Component, instance }));
